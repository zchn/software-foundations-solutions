...
Fixpoint bin2nat (b : bin) : nat :=
  match b with
    | B => O
    | E b' => (plus (bin2nat b') (bin2nat b'))
    | F b' => (plus (plus (bin2nat b') (bin2nat b')) 1)
  end.
...
Example test_bin1: (inc (nat2bin 10)) = nat2bin 11.
Proof. reflexivity. Qed.

Example test_bin2: (nat2bin 0) = B.
Proof. compute. reflexivity. Qed.

Example test_bin3: (nat2bin 1) = F B.
Proof. compute. reflexivity. Qed.

Example test_bin4: (nat2bin 2) = E (F B).
Proof. compute. reflexivity. Qed.

Example test_bin5: (nat2bin 5) = F (E (F B)).
Proof. compute. reflexivity. Qed.

Example test_bin6: (bin2nat (nat2bin 10)) = 10.
Proof. compute. reflexivity. Qed.
